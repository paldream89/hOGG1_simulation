...
# ================= Fourth figure with gradient circles (linear alpha fade from center to edge) =================
fig, ax4 = plt.subplots(figsize=(square_size, square_size))

# Adjustable parameters
max_radius = 1.5  # Maximum radius of the circles
num_layers = 50  # Number of concentric layers for gradient
radii = np.linspace(0, max_radius, num_layers)
alphas = np.linspace(1, 0, num_layers)  # Linear fade from 1 at center to 0 at edge

# Draw circular gradients at each point (start and end of each trajectory)
for i, trajectory in enumerate(all_trajectories):
    x, y = trajectory.T
    color = cm.jet(colors[i])  # Colormap based on displacement

    for px, py in [(x[0], y[0]), (x[-1], y[-1])]:
        for radius, alpha in zip(radii, alphas):
            circ = Circle((px, py), radius=radius, color=color, alpha=alpha, linewidth=0)
            ax4.add_patch(circ)
            
# Draw circular gradients, center dots, and solid outlines
for i, trajectory in enumerate(all_trajectories):
    x, y = trajectory.T
    color = cm.jet(colors[i])
    ...
    for px, py in [(x[0], y[0]), (x[-1], y[-1])]:
        ...
        ax4.plot(px, py, 'o', color='black', markersize=3, zorder=5)

        # Draw black solid outline at max radius
        outline = Circle((px, py), radius=max_radius, fill=False, edgecolor='black', linewidth=1, linestyle='--', zorder=4)
        ax4.add_patch(outline)
...
plt.savefig("brownian_motion_gradient_circles.png", dpi=300, bbox_inches='tight', pad_inches=0)
plt.show()
